/**
 * Minimal synchronous SHA-256. The replay record has to carry a real hash of the
 * requirement bundle so a decision can be proven to have been made against
 * exactly these inputs. crypto.subtle is async and node:crypto is server-only,
 * so the evaluator carries its own.
 */
const K = [
